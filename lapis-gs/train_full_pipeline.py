...
if __name__ == "__main__":
    ...
    for idx, resolution in enumerate(resolution_scales):
        ...
        # if resolution == 8 or resolution == 4 or resolution == 2 or resolution == 1:
        #     continue
        # if resolution == 16:
        #     continue

        if method == "lapis":
            if resolution == resolution_scales[0]:
                train_command = f"python {train_bin} -s {source_dir} -m {model_dir} --data_device cuda --lambda_dssim 0.8"
            else:
                train_command = f"python {train_bin} -s {source_dir} -m {model_dir} --data_device cuda --dynamic_opacity --lambda_dssim 0.8 --foundation_gs_path {model_base}/{dataset_name}/{scene}/{method}/{scene}_res{resolution_scales[idx-1]}/point_cloud/iteration_30000/point_cloud.ply"
        elif method == "freeze":
            if resolution == resolution_scales[0]:
                train_command = f"python {train_bin} -s {source_dir} -m {model_dir} --densify_grad_threshold 0.0015 --data_device cuda --lambda_dssim 0.8"
            else:
                train_command = f"python {train_bin} -s {source_dir} -m {model_dir} --densify_grad_threshold 0.0025 --data_device cuda --lambda_dssim 0.8 --foundation_gs_path {model_base}/{dataset_name}/{scene}/{method}/{scene}_res{resolution_scales[idx-1]}/point_cloud/iteration_30000/point_cloud.ply"
